...
locate_ldr (const void *startAddr)
{
        ...
        void *i = (void *)startAddr;
        int min_addr = xref_target - 0x1000;
        while (1) {
                int len = (char*)i - (char*)min_addr;
                if (len <= 0) break; // Add this check
                i = ldr32_search_up (i, len);
                if (i == 0) {
                        break;
                }
                int dw = *(int *) i;
                int ldr_target = ((char*)i + 4 - ((char*)current_image.image)) & ~3 + ((dw >> 16) & 0xfff);
        ...
        while (1) {
                int len = (char*)i - (char*)min_addr;
                if (len <= 0) break; // Add this check
                i = ldr_search_up (i, len);
                if (i == 0) {
                        break;
                }
                int dw = *(int *) i;
                int ldr_target = ((char*)i + 4 - ((char*)current_image.image)) & ~3 + ((dw & 0xff) << 2);
                if (ldr_target == xref_target) {
                        return i;
                }
                i = (void*)((char*)i - 2);
